...
LOCK_SHA256 = "fd94d092f541f306419f7d4f2087faceff4bd8998a99ffefa9bb39891c77559d"
...
def native_completion_diagnostics(completion):
    ...
    with zipfile.ZipFile(ROOT / 'release/meshia_node-1.3.24-py3-none-any.whl') as archive:
...
def subprocess_diagnostics(argv, returncode, stdout, stderr):
    ...
    script = (ROOT / 'release/install-1.3.24.sh').read_text()
...
def verify_release(root=None):
    ...
    validate_manifest(manifest, lock["source_commit"],
                      lock["artifacts"]["meshia_node-1.3.24-py3-none-any.whl"],
                      lock["artifacts"]["MeshiaNode-1.3.24.app.zip"])
    require(manifest["version"] == lock["version"] == "1.3.24", "Unexpected release version")
...
def fuse_prerequisite_projection():
    ...
    verify_release()
    source = (ROOT / 'release/install-1.3.24.sh').read_text()
