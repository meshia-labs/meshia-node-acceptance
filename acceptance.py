...
LOCK_SHA256 = "048d8573d287759179c32777d17f074664f5e0b1897e5d6068ba1e1ddc374c12"
...
def native_completion_diagnostics(completion):
    ...
    with zipfile.ZipFile(ROOT / 'release/meshia_node-1.3.28-py3-none-any.whl') as archive:
...
def subprocess_diagnostics(argv, returncode, stdout, stderr):
    ...
    script = (ROOT / 'release/install-1.3.28.sh').read_text()
...
def verify_release(root=None):
    ...
    validate_manifest(manifest, lock["source_commit"],
                      lock["artifacts"]["meshia_node-1.3.28-py3-none-any.whl"],
                      lock["artifacts"]["MeshiaNode-1.3.28.app.zip"])
    require(manifest["version"] == lock["version"] == "1.3.28", "Unexpected release version")
...
def fuse_prerequisite_projection():
    ...
    verify_release()
    source = (ROOT / 'release/install-1.3.28.sh').read_text()
