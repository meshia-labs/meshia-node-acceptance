...
LOCK_SHA256 = "04597cc19312e25707267acae0e728ffce6e19798c4629d51ce5fe84addb911b"
...
def native_completion_diagnostics(completion):
    ...
    with zipfile.ZipFile(ROOT / 'release/meshia_node-1.3.21-py3-none-any.whl') as archive:
...
def subprocess_diagnostics(argv, returncode, stdout, stderr):
    ...
    script = (ROOT / 'release/install-1.3.21.sh').read_text()
...
def verify_release(root=None):
    ...
    validate_manifest(manifest, lock["source_commit"],
                      lock["artifacts"]["meshia_node-1.3.21-py3-none-any.whl"],
                      lock["artifacts"]["MeshiaNode-1.3.21.app.zip"])
    require(manifest["version"] == lock["version"] == "1.3.21", "Unexpected release version")
...
def fuse_prerequisite_projection():
    ...
    verify_release()
    source = (ROOT / 'release/install-1.3.21.sh').read_text()
