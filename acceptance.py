...
LOCK_SHA256 = "ab292bc1f64143f287cde1a243206c1aba7f2ece07326f8305c9d3be304d2b5e"
...
def native_completion_diagnostics(completion):
    ...
    with zipfile.ZipFile(ROOT / 'release/meshia_node-1.3.42-py3-none-any.whl') as archive:
...
def subprocess_diagnostics(argv, returncode, stdout, stderr):
    ...
    script = (ROOT / 'release/install-1.3.42.sh').read_text()
...
def verify_release(root=None):
    ...
    require(re.fullmatch(r'[a-f0-9]{64}', LOCK_SHA256 or '') is not None,
    ...
    validate_manifest(manifest, lock["source_commit"],
                      lock["artifacts"]["meshia_node-1.3.42-py3-none-any.whl"],
                      lock["artifacts"]["MeshiaNode-1.3.42.app.zip"])
    require(manifest["version"] == lock["version"] == "1.3.42", "Unexpected release version")
...
def readiness_projection(status):
    ...
    phases = {'before_child_entry', 'child_entry', 'root_import', 'root_setup', 'root_lstat',
    ...
    phase = evidence.get('probe_phase')
    probe = ({'probe_phase': phase} if evidence.get('ready') is False
             and isinstance(phase, str) and phase in phases else {})
    return {
        ...
        'workspace_execution': {'present': bool(evidence),
            ...
            'reason': evidence.get('reason') if evidence.get('reason') in reasons else 'unknown', **probe},
    }
...
def fuse_prerequisite_projection():
    ...
    verify_release()
    source = (ROOT / 'release/install-1.3.42.sh').read_text()
...
def acceptance(directory, *, profile="changed-paths"):
    ...
    def durable_recreated_source(source, destination):
        from meshia_node.config import ConfigStore, Paths
        from meshia_node.client import SignedClient
        from meshia_node.identity import DeviceIdentity
        from meshia_node.fabric import SignedFabricTransport
        import cow_acceptance
        expected = {source: cow_acceptance.CLASSIC_RECREATE_BODY,
                    destination: cow_acceptance.CLASSIC_RENAME_BODY}
        wait('durable recreated source and renamed destination', lambda:
             all(plane.fabric_files.get(path) == body for path, body in expected.items()), 90)
        paths = Paths(node_home)
        store = ConfigStore(paths); current = store.require()
        require(current.api_url == plane.origin and current.host_id in plane.hosts,
                'Recreation observer must use this disposable loopback account')
        transport = SignedFabricTransport(SignedClient(store, DeviceIdentity.load_or_create(paths.identity_dir)),
                                         current.host_id, allow_insecure_loopback=True)
        common = {'attachment_id': current.attachment_id, 'workspace': 'workspace'}
        facts = cow_acceptance.verify_recreated_source_projection(
            transport.snapshot_v2({**common, 'limit': 256}),
            transport.lookup_v2({**common, 'path': source}),
            transport.lookup_v2({**common, 'path': destination}), source=source, destination=destination)
        for path, body in expected.items():
            entry = plane.v2_entries[path]
            actual, _ = plane._v2_file_bytes({'kind': 'file', 'size_bytes': entry['size_bytes'],
                'sha256': entry['sha256'], 'blocks': entry['blocks'], 'modified_at': None})
            require(actual == body, 'Authoritative recreated-source bytes changed')
        return {**facts, 'durable_publication': True, 'recreation_before_ack_timing_tested': False}
