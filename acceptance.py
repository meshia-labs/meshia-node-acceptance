...
LOCK_SHA256 = "9a0e98a7043bc84e31c53cc851d9b4a72afe9b84a9940178335de859b7173b3f"
...
def native_completion_diagnostics(completion):
    ...
    with zipfile.ZipFile(ROOT / 'release/meshia_node-1.3.26-py3-none-any.whl') as archive:
...
def subprocess_diagnostics(argv, returncode, stdout, stderr):
    ...
    script = (ROOT / 'release/install-1.3.26.sh').read_text()
...
def verify_release(root=None):
    ...
    validate_manifest(manifest, lock["source_commit"],
                      lock["artifacts"]["meshia_node-1.3.26-py3-none-any.whl"],
                      lock["artifacts"]["MeshiaNode-1.3.26.app.zip"])
    require(manifest["version"] == lock["version"] == "1.3.26", "Unexpected release version")
...
def fuse_prerequisite_projection():
    ...
    verify_release()
    source = (ROOT / 'release/install-1.3.26.sh').read_text()
