...
LOCK_SHA256 = "d9b65b9cfde71c023791cf07d372c9d3dfafa24bb63e803a39ff9b473a6c688b"
...
def native_completion_diagnostics(completion):
    ...
    with zipfile.ZipFile(ROOT / 'release/meshia_node-1.3.25-py3-none-any.whl') as archive:
...
def subprocess_diagnostics(argv, returncode, stdout, stderr):
    ...
    script = (ROOT / 'release/install-1.3.25.sh').read_text()
...
def verify_release(root=None):
    ...
    validate_manifest(manifest, lock["source_commit"],
                      lock["artifacts"]["meshia_node-1.3.25-py3-none-any.whl"],
                      lock["artifacts"]["MeshiaNode-1.3.25.app.zip"])
    require(manifest["version"] == lock["version"] == "1.3.25", "Unexpected release version")
...
def fuse_prerequisite_projection():
    ...
    verify_release()
    source = (ROOT / 'release/install-1.3.25.sh').read_text()
